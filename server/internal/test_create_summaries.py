#resources/test_create_summary.py
...
class CreateSummaries:
    ...
    def counting_histogram_values(qid,list_of_report_objects):
        ...
        '''
        bins = CreateSummaries.bins_per_qid(qid,list_of_report_objects)
    ...
    def create_summary(surveyid,qid):
        ...
        evaluation = CreateSummaries.counting_histogram_values(qid,reports)
        summed_answers = (evaluation['histogram'])
        count_answers = (evaluation['counts'])

        #fourth: determine other values for generating a summary:
        infos_from_reports = CreateSummaries.extract_from_reports(qid,reports)
        report_name = infos_from_reports['name']
        report_options = infos_from_reports['options']

        infos_from_survey = CreateSummaries.extract_from_surveys(qid,survey)
